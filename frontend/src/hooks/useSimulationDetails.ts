...
import { PcUtilizationChartDatum } from '../types/pc-utilization-chat-datum';

const toPcUtilizationChartData = (pcs: Array<{ id: number; busy_time: number; maintenance_time: number; idle_time: number }>): 
	PcUtilizationChartDatum[] => (
		pcs.map(pc => {
			/* Tiempo total acumulado de la PC */
